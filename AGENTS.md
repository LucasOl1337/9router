...
- xAI Imagine: `xai/grok-imagine-image` and `xai/grok-imagine-image-2.0` in `open-sse/providers/registry/xai.js`. Image requests reuse `grok-cli` OAuth via `credentialFallback` when there is no `xai` sqlite row (`src/sse/handlers/imageGeneration.js`). Do not clone grok-cli tokens into an xai connection.
